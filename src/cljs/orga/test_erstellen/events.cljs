...
(rf/reg-event-db
  ...
  (fn [db _]
    ...
    (-> db
        (assoc-in [:test-erstellen :start-zeit] 0)
        (assoc-in [:test-erstellen :end-zeit] 0))))
    ...
    {:db         db
     :http-xhrio {:method          :post
                  ...
                  :params          {:test-name (get-in db [:test-erstellen :test-name])
                                    :start     (time/input->datetime-str (get-in db [:test-erstellen :start-datum])
                                                                         (get-in db [:test-erstellen :start-zeit]))
                                    :ende      (time/input->datetime-str (get-in db [:test-erstellen :end-datum])
